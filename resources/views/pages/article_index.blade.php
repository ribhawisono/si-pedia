...
<main class="mx-auto max-w-[1440px] px-4 sm:px-6 lg:px-8 py-6 lg:py-8">
  ...
  <div class="hidden lg:grid grid-cols-[60px_120px_1fr_130px_110px_150px_140px_220px] gap-2 rounded-xl bg-tablehead px-4 py-3 text-sm font-bold text-gray-800">
  ...
  <div class="mt-3 space-y-3">
    ...
    <div class="hidden lg:grid grid-cols-[60px_120px_1fr_130px_110px_150px_140px_220px] items-start gap-2 rounded-2xl bg-white px-4 py-4 shadow-[0_2px_10px_rgba(0,0,0,0.06)] overflow-hidden">
      ...
      {{-- Uniform action grid: every button is h-8, full-width of its grid cell,
           laid out 2 per row so buttons always line up horizontally/vertically
           regardless of how many actions this row has. --}}
      <div class="grid grid-cols-2 gap-1.5 min-w-0">
        @if($article->user_id === auth()->id())
        <a href="{{ route('admin.articles.edit', $article) }}"
           class="flex h-8 items-center justify-center rounded-md bg-edit text-xs font-bold text-black">Edit</a>
        @endif

        @if($article->status === 'pending')
          <form action="{{ route('admin.articles.approve', $article) }}" method="POST">
            @csrf @method('PATCH')
            <button class="flex h-8 w-full items-center justify-center rounded-md bg-green-500 text-xs font-bold text-white hover:bg-green-600">✅ Acc</button>
          </form>
          <form action="{{ route('admin.articles.reject', $article) }}" method="POST">
            @csrf @method('PATCH')
            <button class="flex h-8 w-full items-center justify-center rounded-md bg-orange-400 text-xs font-bold text-white hover:bg-orange-500">❌ Tolak</button>
          </form>
        @endif
        ...
        @if($article->status === 'pending_delete')
          <form action="{{ route('admin.articles.approveDelete', $article) }}" method="POST"
            ...
            @csrf @method('DELETE')
            <button class="flex h-8 w-full items-center justify-center rounded-md bg-red-500 text-xs font-bold text-white hover:bg-red-600">🗑 Hapus</button>
          </form>
          <form action="{{ route('admin.articles.rejectDelete', $article) }}" method="POST">
            @csrf @method('PATCH')
            <button class="flex h-8 w-full items-center justify-center rounded-md bg-gray-200 text-xs font-bold text-gray-700 hover:bg-gray-300">↩ Batal</button>
          </form>
        @else
          <form action="{{ route('admin.articles.destroy', $article) }}" method="POST" class="col-span-2"
                onsubmit="return confirm('Pindahkan artikel ini ke Trash? Bisa dipulihkan nanti.')">
            @csrf @method('DELETE')
            <button class="flex h-8 w-full items-center justify-center rounded-md bg-danger text-xs font-bold text-white">Delete</button>
          </form>
        @endif
      </div>
    </div>
