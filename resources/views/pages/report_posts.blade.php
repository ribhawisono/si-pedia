...
<main class="mx-auto max-w-[1440px] px-4 sm:px-8 py-5 sm:py-7">
  <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
    <div><h1 class="text-2xl sm:text-5xl font-black tracking-tight">Report Posts</h1>
      <p class="mt-1 text-sm sm:text-base text-gray-700">Reports and Statistics of All Article Posts on the System.</p></div>
    <div class="flex gap-3"><span class="flex items-center gap-2 rounded-lg border border-gray-300 px-3 py-2 text-xs sm:text-sm font-semibold whitespace-nowrap">01 - {{ \Carbon\Carbon::now()->translatedFormat('d F Y') }}</span></div>
  </div>

  {{-- Stats Cards --}}
  <div class="mt-5 sm:mt-7 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-6">
    <div class="rounded-2xl border border-gray-200 bg-white px-3 sm:px-6 py-4 sm:py-7 text-center shadow-sm">
      <p class="text-xs sm:text-xl font-bold text-gray-900">Total Posts</p>
      <p class="mt-1 text-2xl sm:text-6xl font-black text-gray-900">{{ $stats['total'] }}</p>
    </div>
    <div class="rounded-2xl border border-gray-200 bg-white px-3 sm:px-6 py-4 sm:py-7 text-center shadow-sm">
      <p class="text-xs sm:text-xl font-bold text-gray-900">Active Posts</p>
      <p class="mt-1 text-2xl sm:text-6xl font-black text-green-600">{{ $stats['active'] }}</p>
    </div>
    <div class="rounded-2xl border border-gray-200 bg-white px-3 sm:px-6 py-4 sm:py-7 text-center shadow-sm">
      <p class="text-xs sm:text-xl font-bold text-gray-900">Draft Post</p>
      <p class="mt-1 text-2xl sm:text-6xl font-black text-yellow-600">{{ $stats['draft'] }}</p>
    </div>
    <div class="rounded-2xl border border-gray-200 bg-white px-3 sm:px-6 py-4 sm:py-7 text-center shadow-sm">
      <p class="text-xs sm:text-xl font-bold text-gray-900">Post Deleted</p>
      <p class="mt-1 text-2xl sm:text-6xl font-black text-red-600">{{ $stats['deleted'] }}</p>
    </div>
    <div class="rounded-2xl border border-gray-200 bg-white px-3 sm:px-6 py-4 sm:py-7 text-center shadow-sm">
      <p class="text-xs sm:text-xl font-bold text-gray-900">Scheduled</p>
      <p class="mt-1 text-2xl sm:text-6xl font-black text-blue-600">{{ $stats['scheduled'] }}</p>
    </div>
  </div>

  <div class="mt-6 sm:mt-8 grid grid-cols-1 lg:grid-cols-[1.7fr_1fr] gap-6">
    {{-- Articles Table --}}
    <div class="rounded-2xl border border-gray-200 p-5 shadow-sm overflow-x-auto">
