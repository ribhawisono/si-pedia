<x-layouts.admin title="Report Artikel — SI-Pedia" section="article-reports">
<main class="mx-auto max-w-[1200px] px-4 sm:px-8 py-6 sm:py-10">

  <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
    <div>
      <h1 class="page-title">Laporan Artikel</h1>
      <p class="mt-1 text-gray-500 text-sm">Tinjau dan kelola laporan artikel dari pengguna.</p>
    </div>
  ...
  <div class="mt-6 flex gap-2 sm:gap-3 overflow-x-auto">
    ...
    <a href="{{ route('admin.article-reports.index', $val ? ['status' => $val] : []) }}"
  ...
  <div class="mt-6 space-y-4">
      ...
      <div class="px-4 sm:px-6 py-4 sm:py-5">
        <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
          <div class="flex-1 min-w-0">
            ...
            <div class="mt-3 flex flex-wrap items-center gap-3">
              ...
              <div class="flex items-center gap-2 min-w-0">
                <div class="h-8 w-8 rounded-full bg-red-100 dark:bg-red-900/30 flex items-center justify-center text-xs font-bold text-red-700 dark:text-red-300 flex-shrink-0">
                  📄
                </div>
                <div class="min-w-0">
                  @if($report->article)
                  <a href="{{ route('articles.show', $report->article->slug) }}" class="text-sm font-bold text-gray-900 hover:text-brand-700 truncate block max-w-[220px]">{{ $report->article->title }}</a>
                  <p class="text-xs text-gray-400">Artikel Dilaporkan</p>
                  @else
                  <p class="text-sm font-bold text-gray-900">Artikel dihapus</p>
                  <p class="text-xs text-gray-400">Artikel Dilaporkan</p>
                  @endif
                </div>
              </div>
          ...
          <div class="flex-shrink-0 w-full sm:w-auto">
            <form action="{{ route('admin.article-reports.update', $report) }}" method="POST"
    ...
    <div class="rounded-2xl bg-gray-50 border border-gray-100 p-16 text-center">
      ...
      <p class="font-semibold text-gray-500">Tidak ada laporan artikel.</p>
    </div>
    @endforelse
  </div>
